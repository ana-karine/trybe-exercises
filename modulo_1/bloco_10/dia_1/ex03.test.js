...
describe('myRemoveWithoutCopy', () => {
  test('returns the same array without the item if the item belongs to it', () => {
    ...
    // expect(myRemoveWithoutCopy([1, 2, 3, 4], 3)).not.toEqual([1, 2, 4]); // "not" para "Solução do gabarito"
  });
  ...
  test('returns an identical array if the item does not belong to it', () => {
    ...
    // expect(myRemoveWithoutCopy([1, 2, 3, 4], 5)).not.toEqual([1, 2, 3, 4]); // "not" para "Solução do gabarito"
  });
});
